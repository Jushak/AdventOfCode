...
- The first index which produces a triple is 18, because the MD5 hash of abc18 contains ...cc38887a5.... However, index
18 does not count as a key for your one-time pad, because none of the next thousand hashes (index 19 through
...
namespace Day14
{
    class Program
    {
        ...
        {
            string puzzleInput = "zpqevtbw";
            // Flip to false for part 1.
            bool Part2 = true;
            ...
            using (MD5 md5Hash = MD5.Create())
            {
                ...
                // Generate initial 1000 hashes and store them.
                for ( int i = 0; i < 1000; i++ )
                {
                    hash = GetMd5Hash(md5Hash, puzzleInput + i);
                    if (Part2)
                    {
                        // If in part 2, extra 2016 re-hashes.
                ...
                while (keys.Count < 64)
                {
                    // Add one more hash to the list of hashes.
                    hash = GetMd5Hash(md5Hash, puzzleInput + (index + 1000));
                    if (Part2)
                    {
                        // If in part 2, extra 2016 re-hashes.
                        originalHash = hash;
                    ...
                    // Switch to hash in the current index.
                    hash = hashes[index];
                    ...
                    for (int i = 0; i < hash.Length - 2; i++)
                    {
                        if (hash[i] == hash[i + 1] && hash[i] == hash[i + 2])
                        {
                            ...
                            {
                                string testHash = hashes[index + j];
                                if ( ValidatesKey(hash[i], testHash))
                                {
                                    // If a validating hash is found, add the current hash and end the loop.
                                    if (Part2)
                                    {
                                        keys.Add(originalHash);
                                    }
                                    else
                                    {
                                        keys.Add(hash);
                                    }
                                    Console.WriteLine(hash + " @" + index + " matching hash @"+(index+j) + " is Key #"+keys.Count);
                                    j = 1001;
                                }
                            }
                            // Only need to consider the first triple character set found, so end the loop.
                            i = hash.Length;
                        }
                    }
                    index++;
                ...
                // Roll back one index after done running to get the index where final key was found.
                index--;

                Console.WriteLine("64th key for one-time pad found at index " + index);
            }
        }
        
        // Check if the given hash has set of five of the given character in a row.
        static bool ValidatesKey( char character, string hash )
        {
            for ( int i = 0; i < hash.Length-4; i++)
            {
                if ( hash[i] == character && hash[i + 1] == character && hash[i + 2] == character && 
                    hash[i + 3] == character && hash[i + 4] == character)
                {
                    return true;
                }
            }
