...
3, 4, 1, 5.
    ...
    - The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size increases
    to 3.
...
the previous example was your first round, you would start your second round with the same length sequence 
(3, 4, 1, 5, 17, 31, 73, 47, 23, now assuming they came from ASCII codes and include the suffix), but start with 
...
namespace Day10
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Part 1: " + numbersList[0] * numbersList[1]);
            Console.ReadKey();
            Console.WriteLine();

            // Part 2
            numbersList.Clear();
            // Re-initialize the numbers list.
            for (int i = 0; i < 256; i++)
            {
                numbersList.Add(i);
            }
            lengths.Clear();
            // Convert input characters to lenghts.
            ...
            while (index < 256)
            {
                int item = numbersList[index];
                for (int i = 1; i < 16; i++)
                {
                    item ^= numbersList[index + i];
                }
