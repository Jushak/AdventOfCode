...
namespace Day18
{
    class Program
    {
        static long GetRegistryValue(string id, Dictionary<string, long> registeries)
        {
            if (!registeries.ContainsKey(id))
            {
                registeries.Add(id, 0);
                return 0;
            }
            return registeries[id];
        }

        static void SetRegistryValue(string id, Dictionary<string, long> registeries, long value)
        {
            if (!registeries.ContainsKey(id))
            {
                registeries.Add(id, value);
            }
            else
            {
                registeries[id] = value;
            }
        }

        static void MulRegistryValue(string id, Dictionary<string, long> registeries, long value)
        {
            if (!registeries.ContainsKey(id))
            {
                registeries.Add(id, 0);
        ...
        static void ModRegistryValue(string id, Dictionary<string, long> registeries, long value)
        {
            if (!registeries.ContainsKey(id))
            {
                registeries.Add(id, 0);
        ...
        static void Main(string[] args)
        {
            ...
            /*
            commands.Clear();
            commands.Add("set a 1");
            commands.Add("add a 2");
            commands.Add("mul a a");
            commands.Add("mod a 5");
            commands.Add("snd a");
            commands.Add("set a 0");
            commands.Add("rcv a");
            commands.Add("jgz a -1");
            commands.Add("set a 1");
            commands.Add("jgz a -2");
            */
            ...
            // Part 1:
            Dictionary<string, long> registeries = new Dictionary<string, long>();
            int index = 0;
            ...
            long value = 0;
            long tmp = 0;
            while (index >= 0 && index < count && !recovered)
            {
                input = commands[index].Split(delimiter, StringSplitOptions.RemoveEmptyEntries);
                switch (input[0])
                {
                    case "snd":
                        if (!long.TryParse(input[1], out sound))
                        {
                            sound = GetRegistryValue(input[1], registeries);
                        }
                        index++;
                        break;
                    case "set":
                        ...
                        {
                            value = GetRegistryValue(input[2], registeries);
                        }
                        SetRegistryValue(input[1], registeries, value);
                        index++;
                        break;
                    case "add":
                        tmp = GetRegistryValue(input[1], registeries);
                        if (!long.TryParse(input[2], out value))
                        {
                            value = GetRegistryValue(input[2], registeries);
                        }
                        SetRegistryValue(input[1], registeries, tmp + value);
                        index++;
                        break;
                    case "mul":
                        if (!long.TryParse(input[2], out value))
                        ...
                        MulRegistryValue(input[1], registeries, value);
                        index++;
                        break;
                    case "mod":
                        if (!long.TryParse(input[2], out value))
                        ...
                        ModRegistryValue(input[1], registeries, value);
                        index++;
                        break;
                    case "rcv":
                        if (!long.TryParse(input[1], out value))
                        {
                            value = GetRegistryValue(input[1], registeries);
                        }
                        if (value != 0)
                        {
                            recovered = true;
            ...
            Console.WriteLine("Part 1: The first recovered frequency is " + sound);
            Console.ReadKey();

            // Part 2
            Dictionary<string, long> registery0 = new Dictionary<string, long>();
            Dictionary<string, long> registery1 = new Dictionary<string, long>();
            registery0.Add("p", 0);
            registery1.Add("p", 1);
            ...
            */
            ...
            {
                if (waiting0 && queue0.Count > 0)
                {
                    waiting0 = false;
                }
                if (waiting1 && queue1.Count > 0)
                {
                    waiting1 = false;
                }
                if (waiting0 && waiting1)
                {
                    terminated0 = terminated1 = true;
                }

                if (!waiting0 && !terminated0)
                {
                    input = commands[index0].Split(delimiter, StringSplitOptions.RemoveEmptyEntries);
                    switch (input[0])
                    {
                        case "snd":
                            if (!long.TryParse(input[1], out sound))
                            {
                                sound = GetRegistryValue(input[1], registery0);
                            }
                            queue1.Enqueue(sound);
                            index0++;
                            break;
                        case "set":
                            if (!long.TryParse(input[2], out value))
                            {
                                value = GetRegistryValue(input[2], registery0);
                            }
                            SetRegistryValue(input[1], registery0, value);
                            index0++;
                            break;
                        case "add":
                            tmp = GetRegistryValue(input[1], registery0);
                            if (!long.TryParse(input[2], out value))
                            {
                                value = GetRegistryValue(input[2], registery0);
                            }
                            SetRegistryValue(input[1], registery0, tmp + value);
                            index0++;
                            break;
                        case "mul":
                            if (!long.TryParse(input[2], out value))
                            ...
                            MulRegistryValue(input[1], registery0, value);
                            index0++;
                            break;
                        case "mod":
                            if (!long.TryParse(input[2], out value))
                            ...
                            ModRegistryValue(input[1], registery0, value);
                            index0++;
                            break;
                        case "rcv":
                            if (queue0.Count <= 0)
                            ...
                            {
                                 SetRegistryValue(input[1], registery0, queue0.Dequeue());
                            }
                            index0++;
                            break;
                        case "jgz":
                            if (!long.TryParse(input[1], out value))
                            {
                                value = GetRegistryValue(input[1], registery0);
                            }
                            if (value > 0)
                            {
                                if (!long.TryParse(input[2], out value))
                    ...
                    terminated0 = (index0 < 0 || index0 >= count);
                }
                if (!waiting1 && !terminated1)
                {
                    input = commands[index1].Split(delimiter, StringSplitOptions.RemoveEmptyEntries);
                    switch (input[0])
                    {
                        case "snd":
                            if (!long.TryParse(input[1], out sound))
                            {
                                sound = GetRegistryValue(input[1], registery1);
                            }
                            queue0.Enqueue(sound);
                            sendCount++;
                            index1++;
                            break;
                        case "set":
                            if (!long.TryParse(input[2], out value))
                            {
                                value = GetRegistryValue(input[2], registery1);
                            }
                            SetRegistryValue(input[1], registery1, value);
                            index1++;
                            break;
                        case "add":
                            tmp = GetRegistryValue(input[1], registery1);
                            if (!long.TryParse(input[2], out value))
                            {
                                value = GetRegistryValue(input[2], registery1);
                            }
                            SetRegistryValue(input[1], registery1, tmp + value);
                            index1++;
                            break;
                        case "mul":
                            if (!long.TryParse(input[2], out value))
                            ...
                            MulRegistryValue(input[1], registery1, value);
                            index1++;
                            break;
                        case "mod":
                            if (!long.TryParse(input[2], out value))
                            ...
                            ModRegistryValue(input[1], registery1, value);
                            index1++;
                            break;
                        case "rcv":
                            if (queue1.Count <= 0)
                            ...
                            {
                                SetRegistryValue(input[1], registery1, queue1.Dequeue());
                            }
                            index1++;
                            break;
                        case "jgz":
                            if (!long.TryParse(input[1], out value))
                            {
                                value = GetRegistryValue(input[1], registery1);
                            }
                            if (value > 0)
                            {
                                if (!long.TryParse(input[2], out value))
                    ...
                    terminated1 = (index1 < 0 || index1 >= count);
                }
            }
            Console.WriteLine("Part 2: Program 1 sent value "+sendCount+" times");
            Console.ReadKey();
        }
    }
}
