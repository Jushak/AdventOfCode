...
namespace Day13
{
    class Layer
    {
        ...
        public Layer(int depth, int range, int position, bool reverse)
        {
            this.depth = depth;
            this.range = range;
        ...
        public void Move()
        {
            if (!reverse && position < range-1)
            {
                position++;
            }
            else if (!reverse && position == range-1)
            {
                reverse = true;
                position--;
            }
    ...
    class Program
    {
        static void Reset(Dictionary<int, Layer> layers)
        {
        ...
        static Dictionary<int, Layer> CopyLayers(Dictionary<int, Layer> layers)
        {
            Dictionary<int, Layer> copy = new Dictionary<int, Layer>();
            foreach (KeyValuePair<int, Layer> pair in layers)
            {
                copy.Add(pair.Key, new Layer(pair.Value.depth, pair.Value.range, pair.Value.position, 
                    pair.Value.reverse));
            }
            return copy;
        }

        static bool IsUncaught(int lastLayer, Dictionary<int, Layer> layerLookup)
        {
            //Console.WriteLine("Current delay being checked: " + delay);
            int currentLayer = -1;
            while (currentLayer <= lastLayer)
            {
                currentLayer++;
                if (layerLookup.ContainsKey(currentLayer))
                {
                    if (layerLookup[currentLayer].position == 0)
                    {
                        return false;
                    }
                }
                foreach(Layer layer in layerLookup.Values)
                {
                    layer.Move();
                }
            }
        ...
        {
            Dictionary<int, Layer> layerLookup = new Dictionary<int, Layer>();
            string[] input;
            string line;
            string[] delimiter = { " ", ":" };
            string filePath = Path.GetDirectoryName(System.AppDomain.CurrentDomain.BaseDirectory);

            // Build Dictionary listing depth/range.
            ...
            while ((line = file.ReadLine()) != null)
            {
                ...
                layerLookup.Add(int.Parse(input[0]), new Layer(int.Parse(input[0]), int.Parse(input[1])));
            }

            /*
            layerLookup.Clear();
            layerLookup.Add(0, new Layer(0, 3));
            layerLookup.Add(1, new Layer(1, 2));
            layerLookup.Add(4, new Layer(4, 4));
            layerLookup.Add(6, new Layer(6, 4));
            */

            int lastLayer = layerLookup.Keys.Max();
            int currentLayer = -1;
            int severity = 0;
            while (currentLayer <= lastLayer)
            {
                ...
                if (layerLookup.ContainsKey(currentLayer))
                {
                    if (layerLookup[currentLayer].position == 0)
                    {
                        severity += layerLookup[currentLayer].depth * layerLookup[currentLayer].range;
                        Console.WriteLine("Caught! Depth " + currentLayer + " and range " + 
                            layerLookup[currentLayer].range + ". Severity is now "+severity);
                    }
                }
                for (int i = currentLayer; i <= lastLayer; i++)
                {
                    if (layerLookup.ContainsKey(i))
                    {
                        layerLookup[i].Move();
                    }
                }
            ...
            Console.WriteLine("Part 1: Severity of the trip is "+severity);
            Console.ReadKey();
            bool uncaught = false;
            int delay = 0;
            ...
            while (!uncaught)
            {
                ...
                uncaught = IsUncaught(lastLayer, CopyLayers(layerLookup));
            }
            Console.WriteLine("Part 2: Smallest delay to remain uncaught is " + delay);
            Console.ReadKey();
        }
    }
}
