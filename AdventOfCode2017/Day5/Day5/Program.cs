...
namespace Day5
{
    class Program
    {
        ...
        {
            List<int> commands1 = new List<int>();
            string line = "";
            ...
            {
                commands1.Add(int.Parse(line));
            }

            List<int> commands2 = new List<int>(commands1);
            int index = 0;
            int steps = 0;
            while (index >= 0 && index < commands1.Count)
            {
                int lastIndex = index;
                index += commands1[index];
                commands1[lastIndex]++;
                steps++;
            }
            ...
            index = 0;
            steps = 0;
            while (index >= 0 && index < commands2.Count)
            {
                int lastIndex = index;
                index += commands2[index];
                if (commands2[lastIndex] >= 3)
                {
                    commands2[lastIndex]--;
                }
                else
