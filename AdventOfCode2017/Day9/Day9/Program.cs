...
group or garbage. Since groups can contain other groups, a } only closes the most-recently-opened unclosed group - 
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < line.Length; i++)
            {
                if (!inGarbage)
                {
                    ...
                    {
                        totalPoints += currentGroupPoints;
                        currentGroupPoints--;
                    }
                    else if (line[i].Equals('<'))
                    {
                    ...
                    {
                        Console.Error.WriteLine("Input failure! Found character '" + line[i] + "'");
                        Console.ReadKey();
                        Environment.Exit(0);
                    }
                }
            ...
            Console.WriteLine("Part 1: Total group points is: "+totalPoints);
            Console.WriteLine("Part 2: Total garbage rid of: " + garbageCount);
            Console.ReadKey();
        }
    }
}
